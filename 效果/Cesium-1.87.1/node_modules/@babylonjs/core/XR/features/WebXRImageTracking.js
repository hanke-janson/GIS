...
var WebXRImageTracking = /** @class */ (function (_super) {
    ...
    options) {
        ...
        _this._trackableScoresReceived = false;
    ...
    WebXRImageTracking.prototype._checkScores = function () {
        if (!this._xrSessionManager.session.getTrackedImageScores || this._trackableScoresReceived) {
            return;
        }
        var imageScores = this._xrSessionManager.session.getTrackedImageScores();
        // check the scores for all
        for (var idx = 0; idx < imageScores.length; ++idx) {
            if (imageScores[idx] == "untrackable") {
                this.onUntrackableImageFoundObservable.notifyObservers(idx);
            }
            else {
                var originalBitmap = this._originalTrackingRequest[idx].image;
                var imageObject = {
                    id: idx,
        ...
        this._trackableScoresReceived || (this._trackableScoresReceived = imageScores.length > 0);
    };
    /**
     * The module's name
     */
